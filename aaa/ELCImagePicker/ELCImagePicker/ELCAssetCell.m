...
- (void)setAssets:(NSArray *)assets
{
    ...
    for (int i = 0; i < [_rowAssets count]; ++i) {
        ...
        if (i < [_overlayViewArray count]) {
            ...
            if (overlayImage == nil) {
                overlayImage = [UIImage imageNamed:@"圆角矩形-1-拷贝"];
            }
...
- (void)cellTapped:(UITapGestureRecognizer *)tapRecognizer
{
    ...
    CGFloat totalWidth = c * ([UIScreen mainScreen].bounds.size.width - 8) / 4 + (c - 1) * 2;
    CGFloat startX;
    
    if (self.alignmentLeft) {
        startX = 4;
    }else {
        startX = ([UIScreen mainScreen].bounds.size.width - totalWidth) / 2;
    }
    
    CGRect frame = CGRectMake(0, 2, ([UIScreen mainScreen].bounds.size.width - 8) / 4, ([UIScreen mainScreen].bounds.size.width - 8) / 4);
